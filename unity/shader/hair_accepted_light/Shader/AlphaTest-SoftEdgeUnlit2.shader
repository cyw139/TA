...
Shader "Legacy Shaders/Transparent/Cutout/Soft Edge Unlit2" {
Properties {
	_Color ("Main Color", Color) = (1, 1, 1, 1)
	_MainTex ("Base (RGB) Alpha (A)", 2D) = "white" {}
	_Cutoff ("Base Alpha cutoff", Range (0,.9)) = .5
	_SoftTransform ("_SoftTransform", Range (0,3)) = 1
}
...
SubShader {
	...
	Pass {
		...
		CGPROGRAM
			...
			{
				half4 col = _Color * tex2D(_MainTex, i.texcoord);
				col.a = pow(col.a - 0.1,_SoftTransform);
				UNITY_APPLY_FOG(i.fogCoord, col);
				return col;
			}
		ENDCG
